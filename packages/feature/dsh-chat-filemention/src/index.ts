/**
 * '@' chat-filemention plugin, host half: two loopback HTTP routes on the Web GUI
 * server that the browser source (src/client) drives while composing a prompt.
 *
 *   GET /chat-filemention/search?session=<id>&q=<query>
 ...
 *   GET /chat-filemention/read?session=<id>&path=<relative>
 ...
 *
 * @module @neplich/dsh-chat-filemention
 */
...
export const name = 'chat-filemention'
...
export function apply(ctx: Context, config: Config) {
  ...
  }), 'chat-filemention: search route')

  ctx.effect(() => webServer.register({
    kind: 'exact',
    path: '/chat-filemention/read',
    async handler(req, res) {
      const url = preflight(req, res)
      if (url === undefined) return
      const cwd = cwdOf(url.searchParams.get('session'))
