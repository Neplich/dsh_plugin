/**
 * '@' chat-filemention plugin, browser half: registers the 'file' source on the
...
export function createFileSource(): { source: InputTriggerSource, reset: () => void } {
  ...
  const search = (sessionId: string, query: string, signal: AbortSignal): Promise<SearchResponse> =>
    fetchJson<SearchResponse>(`/chat-filemention/search?${new URLSearchParams({ session: sessionId, q: query })}`, signal)
  ...
  const source: InputTriggerSource = {
    ...
    codec: {
      ...
      async serialize(ref, signal) {
        ...
        const data = await fetchJson<ReadResponse>(`/chat-filemention/read?${params}`, signal)
...
export function apply(ctx: ClientContext): void {
  ...
  ctx.effect(() => inputTriggers.registerSource(source), 'chat-filemention: @ file source')
}
