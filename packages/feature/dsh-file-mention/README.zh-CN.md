...
- **Host 半**（`src/index.ts`）：在 Web GUI 服务器（`ctx.webServer`）上注册两个同源路由：
  - `GET /file-mention/search?session=<id>&q=<query>` — 遍历会话 cwd（有边界、有忽略清单、按 cwd 单飞缓存），返回列表（空查询：上限 `maxListed`，带 `complete` 标志，客户端据此转为本地过滤）或排序后的 top 匹配（非空查询：超大目录树的客户端回退方案）。
  - `GET /file-mention/read?session=<id>&path=<relative>` — 返回单个文件的文本，限定在会话 cwd 内（词法 + realpath 逃逸检查）、大小受限、拒绝二进制。
