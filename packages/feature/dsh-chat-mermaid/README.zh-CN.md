...
markdown 渲染管线没有代码块的扩展插槽，因此浏览器半采用纯 DOM 升级：用 MutationObserver 扫描 `.md-code-block`，从横幅的语言标记读取代码块语言，把 mermaid 块替换为图表卡片。流式输出中的代码块在落稳（shell 此时才标注语言）之前不做处理。宿主半从插件自身依赖中读取 `mermaid/dist/mermaid.min.js`，通过 `GET /dsh-chat-mermaid/mermaid.min.js`（仅回环、同源）提供，因此客户端 bundle 保持很小。
