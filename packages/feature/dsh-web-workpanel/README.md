...
| Route | Purpose |
|---|---|
| `GET /web-workpanel/list?session=<id>&path=<rel>` | One directory's children (dirs first), capped, confined to the session cwd |
| `GET /web-workpanel/read?session=<id>&path=<rel>` | One text file's content, size-bounded, binary-refused |
| `GET /web-workpanel/raw?session=<id>&path=<rel>` | One allowlisted image, PDF, or modern Office file for inline preview, size-bounded |
| `GET /web-workpanel/pdfjs/*` | Allowlisted PDF.js runtime, worker, viewer, font, color-map, WASM, and image resources |
| `GET /web-workpanel/ooxml/*` | Allowlisted OOXML viewer modules and parser WASM resources |
| `POST /web-workpanel/terminal/close?session=<id>&terminal=<id>` | Close one terminal tab's PTY |
| `WS /web-workpanel/terminal?session=<id>&terminal=<id>&cols=<n>&rows=<n>` | One terminal tab's interactive PTY stream |
...
```sh
dsh plugin --profile <name> add @neplich/dsh-web-workpanel   # or a local path: ./packages/feature/dsh-web-workpanel
...
```sh
dsh plugin --profile <name> remove @neplich/dsh-web-workpanel
```
