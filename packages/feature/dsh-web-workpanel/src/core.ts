/**
 * Node-side core of @neplich/dsh-web-workpanel: path confinement, directory
 ...
 *
 * @module @neplich/dsh-web-workpanel/core
 */
...
export function pdfJsAssetPath(pathname: string): string | undefined {
  const prefix = '/web-workpanel/pdfjs/'
...
export function ooxmlAssetPath(pathname: string): string | undefined {
  const prefix = '/web-workpanel/ooxml/'
  if (!pathname.startsWith(prefix)) return undefined
  let relative: string
  try {
    relative = decodeURIComponent(pathname.slice(prefix.length))
