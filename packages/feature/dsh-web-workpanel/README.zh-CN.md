...
| 路由 | 用途 |
|---|---|
| `GET /web-workpanel/list?session=<id>&path=<rel>` | 列出一个目录的直接子项（目录在前），限制行数，约束在会话 cwd 内 |
| `GET /web-workpanel/read?session=<id>&path=<rel>` | 读取一个文本文件内容，限制大小，拒绝二进制 |
| `GET /web-workpanel/raw?session=<id>&path=<rel>` | 输出允许的图片、PDF 或现代 Office 文件用于内联预览，限制大小 |
| `GET /web-workpanel/pdfjs/*` | 输出白名单内的 PDF.js 运行时、Worker、Viewer、字体、字符映射、WASM 与图片资源 |
| `GET /web-workpanel/ooxml/*` | 输出白名单内的 OOXML 查看器模块与解析器 WASM 资源 |
| `POST /web-workpanel/terminal/close?session=<id>&terminal=<id>` | 关闭一个终端标签对应的 PTY |
| `WS /web-workpanel/terminal?session=<id>&terminal=<id>&cols=<n>&rows=<n>` | 一个终端标签的交互式 PTY 流 |
...
```sh
dsh plugin --profile <name> add @neplich/dsh-web-workpanel   # 或本地路径：./packages/feature/dsh-web-workpanel
...
在仓库根的多插件工作区中构建：`pnpm run build` 产出 `lib/`（tsc），`pnpm --filter @neplich/dsh-web-workpanel run bundle` 产出浏览器 bundle `lib/client.js`（tsdown；react 与平台 client 库经 GUI 模块表解析，xterm 内联），`pnpm run test` 运行包级 vitest 测试。PDF.js 与 OOXML JS/WASM 资源不进入客户端首屏包，只在打开对应文件时从插件同源路由加载。
