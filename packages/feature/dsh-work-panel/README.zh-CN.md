...
面板占用 shell 独立的右侧 workbench 列，打开时会让会话区变窄，不会覆盖在会话内容上；切换工具不会改变列宽。从左边缘拖拽调宽（或将焦点移到拖拽柄上用方向键调整），面板头部提供展开/恢复按钮。窗口宽度低于 shell 自身的自动收起断点时，面板会自动收起，避免挤压会话区。面板与 shell 自带的工具详情栏共用右边缘、互斥存在：打开其中一个会关闭另一个，因此现有工具详情始终可以访问。
...
client 半注册到两个 slot：面板本体进 `shell.workbench`，开关按钮进 `conversation.session.header.utilities`，顺序位于 `Session log` 之后。面板状态（开关、宽度、按会话保存的混合标签栏，以及每个文件标签的浏览状态）保存在一个 root 作用域的 slot store 中。
...
- **需要 workbench 插槽。** 面板依赖 dsh shell 提供 `shell.workbench` 插槽。旧版 dsh 只提供悬浮层，外部插件无法在不修改 shell 的前提下获得真正的第四布局列。
