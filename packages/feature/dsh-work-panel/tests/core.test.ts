...
describe('pdfJsAssetPath', () => {
  it('allows the runtime and one-level support assets', () => {
    expect(pdfJsAssetPath('/work-panel/pdfjs/build/pdf.mjs')).toBe('build/pdf.mjs')
    expect(pdfJsAssetPath('/work-panel/pdfjs/cmaps/78-EUC-H.bcmap')).toBe('cmaps/78-EUC-H.bcmap')
    expect(pdfJsAssetPath('/work-panel/pdfjs/web/images/loading-icon.gif')).toBe('web/images/loading-icon.gif')
  })

  it('rejects arbitrary package files and traversal', () => {
    expect(pdfJsAssetPath('/work-panel/pdfjs/package.json')).toBeUndefined()
    expect(pdfJsAssetPath('/work-panel/pdfjs/cmaps/../package.json')).toBeUndefined()
    expect(pdfJsAssetPath('/work-panel/pdfjs/cmaps/%2e%2e%2fpackage.json')).toBeUndefined()
  })
})

describe('ooxmlAssetPath', () => {
  it('allows browser entries, hashed chunks and parser WASM', () => {
    expect(ooxmlAssetPath('/work-panel/ooxml/docx.mjs')).toBe('docx.mjs')
    expect(ooxmlAssetPath('/work-panel/ooxml/line-metrics-Ab_C-12.js')).toBe('line-metrics-Ab_C-12.js')
    expect(ooxmlAssetPath('/work-panel/ooxml/xlsx_parser_bg.wasm')).toBe('xlsx_parser_bg.wasm')
  })

  it('rejects package metadata, nested files and traversal', () => {
    expect(ooxmlAssetPath('/work-panel/ooxml/package.json')).toBeUndefined()
    expect(ooxmlAssetPath('/work-panel/ooxml/types/index.js')).toBeUndefined()
    expect(ooxmlAssetPath('/work-panel/ooxml/%2e%2e%2fpackage.json')).toBeUndefined()
  })
})
