...
The expand bar copy (collapsed/expanded counts) ships in zh and en through the `chat-autofold` locale namespace and re-renders live when dsh's UI language switches (a `ctx.locale` revision subscription re-paints every mounted bar).
...
```sh
dsh plugin --profile <name> add @neplich/dsh-chat-autofold   # or a local path: ./packages/feature/dsh-chat-autofold
...
Built with the multi-plugin workspace at the repository root: `pnpm run build` emits `lib/` (tsc), `pnpm --filter @neplich/dsh-chat-autofold run bundle` emits the browser bundle `lib/client.js` (tsdown), `pnpm run test` runs the package-level vitest suites.
...
- **Expand state is per page load** — the "user expanded this turn" memory lives in the page; reopening or refreshing the session chat-autofolds the same turn again (this is the default policy, not a bug).
