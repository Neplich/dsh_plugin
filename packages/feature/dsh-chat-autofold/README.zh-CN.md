...
展开条文案（折叠/展开计数）通过 `chat-autofold` locale namespace 提供中英双语，并在 dsh 界面语言切换时实时重绘（`ctx.locale` revision 订阅会重绘所有已挂载的展开条）。
...
```sh
dsh plugin --profile <name> add @neplich/dsh-chat-autofold   # 或本地路径：./packages/feature/dsh-chat-autofold
...
使用仓库根部的多插件工作区构建：`pnpm run build` 产出 `lib/`（tsc），`pnpm --filter @neplich/dsh-chat-autofold run bundle` 产出浏览器 bundle `lib/client.js`（tsdown），`pnpm run test` 运行包级 vitest 套件。
