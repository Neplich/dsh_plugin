...
export function apply(ctx: ClientContext): void {
  ctx.effect(() => ctx.locale.register(NS, { zh, en }), 'dsh-chat-annotations: dictionaries')
  ...
  ctx.effect(() => {
    const disposers = [
      ...
      ctx.slots.inject('conversation.input.left', () => ctx.slots.register(
        { name: 'conversation.input.left', id: 'dsh-chat-annotations', order: 100, locale: NS, inject: () => face() },
        ChipView,
      )),
    ]
    return () => { for (const d of disposers) d() }
  }, 'dsh-chat-annotations: slots')
}
