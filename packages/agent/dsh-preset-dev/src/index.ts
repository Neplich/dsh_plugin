...
export function apply(ctx: Context): void {
  ...
  void installPreset(dest, { force }).then(async (result) => {
    if (result === 'installed') {
      ctx.logger.info(`preset-dev: installed the dev preset at ${dest}`)
    } else if (result === 'updated') {
      ctx.logger.info(`preset-dev: re-installed the dev preset at ${dest}`)
    } else if (await bundledCopyDiffers(dest)) {
      ctx.logger.warn(
        'preset-dev: the installed dev preset differs from this bundle (locally edited or upgraded); '
        + 'run with DSH_PRESET_DEV_FORCE=1 to re-install the bundled version',
      )
    }
  }).catch((error: unknown) => {
    ctx.logger.error(`preset-dev: failed to install the dev preset: ${String(error)}`)
  })
}
